...
export default function useFormValidator() {
  ...
  function handleChange(e) {
    const { name, value, validationMessage, validity: { valid }, form } = e.target;
  
    setValues(initialValues => ({ ...initialValues, [name]: value }));
    setErrors(initialErrors => ({ ...initialErrors, [name]: validationMessage }));
    setIsInputValid(initialValidity => ({ ...initialValidity, [name]: valid }));
    setIsValid(form.checkValidity());
  }
